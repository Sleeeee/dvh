...
void test_Lab_SWD_Solve_Flag_One_should_copy(void) {
  const char mock_flag_one[] = "DVH{m0ck_fl4g_0n3_ffffffffff}";
  Lab_SWD_Solve_Flag_One(mock_flag_one, buffer);

  TEST_ASSERT_EQUAL_STRING(mock_flag_one, (char*)buffer);
}

void test_Lab_SWD_Solve_Flag_Two_should_decrypt(void) {
  ...
  Lab_SWD_Solve_Flag_Two(mock_flag_two_encrypted, sizeof(mock_flag_two_encrypted), buffer);

  TEST_ASSERT_EQUAL_STRING(mock_flag_two_plain, (char*)buffer);
}

void test_Lab_SWD_Solve_Flag_Three_should_decrypt(void) {
  ...
  Lab_SWD_Solve_Flag_Three(mock_flag_three_encrypted, sizeof(mock_flag_three_encrypted), buffer);
